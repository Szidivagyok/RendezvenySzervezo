  ...
  <div>
    <Modal ref="modal" :title="title" @yesEvent="yesEventHandler">
      ...
      <div class="mb-4 row pt-2">
        <label for="serviceTypeName" class="col-form-label col-auto pt-1 pe-0"
          >Szolgáltatás név:</label
        >
        <div class="col">
          <input
            type="text"
            class="form-control"
            id="serviceTypeName"
            v-model="formItem.serviceTypeName"
            @input="clearError('serviceTypeName')"
            required
          />
          <div v-if="!serverErrors.serviceTypeName" class="invalid-feedback position-absolute">
            A szolgáltatás név kötelező
          </div>
          <div v-if="serverErrors.serviceTypeName"
            class="invalid-feedback position-absolute d-block"
          >
            {{ serverErrors.serviceTypeName[0] }}
          </div>
        </div>
      </div>
    </Modal>
